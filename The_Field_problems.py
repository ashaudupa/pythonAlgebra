...
def myFilter(L, num):
    ...
    Examples:
      ...
      [15, 25]
    '''
    return [L[i] for i in range(len(L)) if L[i] % num !=0]
...
def my_lists(L):
    ...
    [[], [1, 2, 3]]
    '''
    return [[x+1 for x in range(L[i])] for i in range(len(L))]
...
def myFunctionComposition(f, g):
    ...
    Examples:
      ...
      True
    '''
    return {x:z for (x,m) in f.items() for (y,z) in g.items() if m == y}
...
a = complex(3,1)
b = complex(2,2)
complex_addition_a =a + b
a = complex(-1,2)
b = complex(1,-1)
complex_addition_b = a + b
a = complex(2,0)
b = complex(-3,.001)
complex_addition_c = a + b
a = complex(0,2)
b = complex(.001,1)
complex_addition_d = 4 * a+b

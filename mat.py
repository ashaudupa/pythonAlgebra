...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    mat_list = [A[(x,y)] == B[(x,y)] for x in A.D[0] for y in A.D[1]]
    for x in mat_list:
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D,{(m,n):M[m,n] * x for m in M.D[0] for n in M.D[1]})
...
def transpose(M):
    ...
    True
    """
    t_dict = {(y,x):M[x,y] for x in M.D[0] for y in M.D[1]}
    return Mat((M.D[1],M.D[0]),t_dict)
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    vec_mat = {col:Vec(M.D[0],{row:M[row,col] for row in M.D[0]})for col in M.D[1]}
    mul_vec = {c:vec_mat[c]*v for c in vec_mat.keys()}
    return Vec(set(mul_vec.keys()),mul_vec)
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    vec_mat = {row:Vec(M.D[1],{col:M[row,col] for col in M.D[1]})for row in M.D[0]}
    mul_vec = {c:vec_mat[c]*v for c in vec_mat.keys()}
    return Vec(set(mul_vec.keys()),mul_vec)
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    row_data = {row:Vec(A.D[1],{col:A[row,col] for col in A.D[1]})for row in A.D[0]}
    col_data = {col:Vec(B.D[0],{row:B[row,col] for row in B.D[0]})for col in B.D[1]}
    return Mat((A.D[0],B.D[1]),{(row,col):row_data[row]*col_data[col] for row in A.D[0] for col in B.D[1]})

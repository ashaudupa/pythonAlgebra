...
def create_voting_dict(strlist):
    ...
    for l in strlist:
        linelist = l.split()
        vote_list = list()
        for words in linelist[3:]:
            vote_list.append(int(words))
        voting_dict.update({linelist[0]:vote_list})
    return voting_dict 
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    voting_dot_product = 0
    for i in range(len(voting_dict[sen_a])):
        voting_dot_product = voting_dot_product + (voting_dict[sen_a][i] * voting_dict[sen_b][i])
    return voting_dot_product 
...
def most_similar(sen, voting_dict):
    ...
    most_sim = 0
    most_sim_item = sen
    for item in voting_dict:
        p_compare = policy_compare(sen,item,voting_dict)
        if(p_compare>=most_sim)and item != sen:
            most_sim = p_compare
            most_sim_item = item
    return most_sim_item 
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'b'
    """
    similar_policy = {s:policy_compare(sen,s,voting_dict) for s in voting_dict if s!=sen}
    inverse = [(value,key) for (key,value) in similar_policy.items()]
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    t_similar = 0
    for item in sen_set:
        t_similar = t_similar + policy_compare(sen,item,voting_dict)
    
    return t_similar/len(sen_set)
...
def find_average_record(sen_set, voting_dict):
    ...
    temp = sen_set.pop()
    sen_set.add(temp)
    for x in range(len(voting_dict[temp])):
        avg_rec.append(sum([voting_dict[y][x] for y in sen_set])/len(sen_set))
    return avg_rec 
...
def bitter_rivals(voting_dict):
    ...
    res = [sens[0], sens[1], policy_compare(sens[0], sens[1], voting_dict)] # starting point
    for sen1 in sens:
        for sen2 in sens:
           pc = policy_compare(sen1, sen2, voting_dict)
           if pc < res[2]:
               res = [sen1, sen2, pc]

...
def equal(u,v):
    ...
    for d in u.D:
        if (d in u.f)and(d in v.f):
            if(u.f[d] != v.f[d]):
                return False
        elif (d in u.f)and(d not in v.f):
            if(u.f[d] != 0):
                return False
...
def dot(u,v):
    ...
    for d in u.D:
        if (d in u.f) and (d in v.f):
            dot_product = dot_product + (u.f[d]*v.f[d])
    return dot_product
...
def scalar_mul(v, alpha):
    ...
    True
    """
    """scalar_product = Vec(v.D,{})
    for d in v.D:
        if d in v.f:
            scalar_product.f[d] = alpha * v.f[d]
    return scalar_product
    """
    #print (v.D)
    #print (v.f)
    return Vec(v.D,{d:v.f[d]*alpha for d in v.D})

...
def movie_review(name):
	...
	review_option = ["A super","A gem","Ideological claptrap","must see","better than last movie"]
	return name + ' is ' + review_option[randint(0,len(review_option)-1)] + ' movie'
...
def makeInverseIndex(strlist):
    ...
    True
    """
	#L = ['hello world','hello','hello cat','hellolot of cats']
	strlist = [strlist[i].split() for i in range(len(strlist))]
	strlist = list(enumerate(strlist))
	d = dict()
	for (indx,word) in strlist:
		for words in word:
			if words in d.keys():
				d[words].update({indx})
			else:
...
def orSearch(inverseIndex, query):
    ...
    s = set()
    for words in query:
        if words in inverseIndex.keys():
            s = s |(inverseIndex[words])
    return s
